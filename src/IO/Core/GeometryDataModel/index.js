    ...
    loadScene(scene) {
        ...
        if(scene) {
            ...
            scene.forEach((obj) => {
                ...
                this.dataMapping[url] = { name, field: 'points', type: obj.points.split('.').slice(-1)[0] };
                urls.push(url);


                url = this.basepath + obj.index;
                this.dataMapping[url] = { name, field: 'index', type: obj.index.split('.').slice(-1)[0] };
                urls.push(url);
                ...
                if(this.coloByMapping[name]) {
                    ...
                    this.dataMapping[url] = { name, field: 'field', type: obj.fields[this.coloByMapping[name]].split('.').slice(-1)[0] };
                    urls.push(url);
                }
