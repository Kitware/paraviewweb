  ...
  loadClusters(queryDataModel, clusterData) {
    if (clusterData.dataset) {
      ...
      for (let i = 0; i < dataSize; i += 7) {
        console.log(
          `(${i / 7}) add cluster center(${clusters[i]}, ${clusters[i + 1]}, ${
            clusters[i + 2]
          })`,
          `- const(${clusters[i + 3]}) - slope(${clusters[i + 4]}, ${
            clusters[i + 5]
          }, ${clusters[i + 6]})`
        );
        this.slicSource.addCluster(
          clusters[i],
          clusters[i + 1],
          clusters[i + 2],
          clusters[i + 3],
      ...
      this.currentImageData
        ...
        .setScalars(
          this.currentImageData
            .getPointData()
            .getArray(this.slicSource.getScalarArrayName())
        );
      this.geometryReady(this.currentImageData);
    }
  }
