...
export default function({client, filterQuery, mustContain, busy}) {
    return {
        ...
        createCluster(cluster) {
            return busy(client._.post(`/clusters`, cluster, {transformRequest, headers}))
        },
        ...
        updateCluster(cluster) {
            const editableCluster = deepClone(cluster),
                ...
                { missingKeys, promise } = mustContain(cluster, ...expected);
            ...
            return missingKeys ? promise :
                        busy(client._.patch(`/clusters/${cluster._id}`, cfiltered, {transformRequest, headers} ));
        },
