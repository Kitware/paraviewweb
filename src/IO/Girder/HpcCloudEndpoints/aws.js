...
function getUnauthenticatedPromise(client) {
    ...
    return new Promise((ok,ko) => ko({ data: { message: 'Must be logged in.'}}));
}

export default function({client, filterQuery, mustContain, busy}) {
    return {
        listAWSProfiles() {
            return getUnauthenticatedPromise(client)
        ...
        listAWSRunningInstances(id) {
            return getUnauthenticatedPromise(client)
                || busy(client._.get(`/user/${client.user._id}/aws/profiles/${id}/runninginstances`));
        },

        getAWSMaxInstances(id) {
            return getUnauthenticatedPromise(client)
                || busy(client._.get(`/user/${client.user._id}/aws/profiles/${id}/maxinstances`));
        },
