...
function generateModel(list, rootId) {
    const model = {
        ...
        tree: { [rootId]:[] }, map: {}, leaves: [],

        // Helper variables
        rootId, y: 0,

        // Results
        nodes: [], forks: [], branches: [], actives: [],
    };
...
export default React.createClass({
    ...
    renderNodes() {
        return this.state.nodes.map((el, index) => {
            ...
            return (<g key={'node-' + index} className={ style.cursor }>
                <circle data-id={el.y}
                    cx={cx}
                    cy={cy}
                    r={radius}
                ...
                <text className={ style.regularText }
                    data-id={el.y}
                    x={tx}
                    y={ty}
    ...
    renderActives() {
        ...
        return this.state.actives.map( (el, index) => {
            return <rect key={'active-' + index} data-id={this.state.nodes[el].y}
                        x='-50' width='1000' fill='#999'
    ...
    toggleActive(event) {
        ...
        if(event.target.nodeName !== 'circle' && !event.target.classList.contains(style.iconText)) {
            ...
            if (this.props.multiselect && (event.metaKey || event.ctrlKey)) { // command key for osx, control key for windows
                if (index === -1) {
                    actives.push(yVal);
                } else {
