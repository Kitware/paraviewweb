...
/**\brief Map points from the unbounded hyperbolic plane to the unit Poincaré disk.
  ...
  * operations involved.
  */
export function hyperbolicPlanePointsToPoincareDisk(xh, focus) {
  // Translate all the points in xh to the focal point:
  const tx = xh.map(pt => pt.map((coord, ii) => coord - focus[ii]));
  // Convert from cartesian coordinates to a radius and scale it to the disk:
  rho = tx.map(pt => Math.sqrt(pt[0]*pt[0] + pt[1]*pt[1]));
  rr = rho.map(rho_ii => rho_ii < 1e-8 ? 0.0 : Math.tanh(rho_ii / 2.0));
  // Use similar triangles to map the translated points to the unit disk:
  xd = rr.map((rr_ii, ii) => rho[ii] < 1e-8 ? [0.0, 0.0] : tx[ii].map(tx_ii => rr_ii * tx_ii / rho[ii]));
  return xd;
}
...
/**\brief Intersect 2 parametric lines in 2D, non-robustly (i.e., the lines are in general position).
  ...
  */
function lineLineIntersectFlimsy(p0, d0, p1, d1) {
...
/**\brief Intersect a parametric line with a circle in 2D, non-robustly (i.e., general position).
  ...
  */
function lineCircleIntersectFlimsy(pp, dd, cc, rr) {
...
/**\brief Compute the path of a geodesic between \a p0 and \a p1 projected to the unit Poincaré disk centered at \a focus.
  ...
  */
export function hyperbolicPlaneGeodesicOnPoincareDisk(p0, p1, focus) {
  ...
  // Translate all the points in xh to the focal point:
  const tx0 = p0.map(pt => pt.map((coord, ii) => coord - focus[ii]));
  const tx1 = p1.map(pt => pt.map((coord, ii) => coord - focus[ii]));
  const pts = zip([tx0, tx1]);
  const paths = pts.map(ppr => {
    let rho = [0, 0];
    let rr = [0, 0];
    let xd = [null, null];
    let bdy = [false, false]; // Does endpoint i lie on the disk boundary?
    for (let ii = 0; ii < 2; ++ii) {
      ...
      if (rho[ii] < 1e-8) {
        ...
        xd[ii] = ppr[ii].map(pt_ii => rr[ii] * pt_ii / rho[ii]);
        bdy[ii] = rr[ii] === 1.0;
      }
    }
    ...
    if (pointsApproxCollinear(xd[0], xd[1], [0.0, 0.0], 1e-8)) {
      ...
      let center = [0., 0.]; // Center of circle forming geodesic between xd[0] & xd[1].
      if (bdy[0] && bdy[1]) {
        // Case where both points on disk boundary. Intersect tangents to get center.
        center = lineLineIntersectFlimsy(xd[0], [xd[0][1], -xd[0][0]], xd[1], [xd[1][1], -xd[1][0]]);
      } else {
        ...
        const cpts = lineCircleIntersectFlimsy(ipt, [ipt[1], -ipt[0]], [0, 0], 1.0);
        // The intersection of the tangent lines at the 2 cpts is a third point on the circle
        const p3 = lineLineIntersectFlimsy(cpts[0], [cpts[0][1], -cpts[0][0]], cpts[1], [cpts[1][1], -cpts[1][0]]);
        ...
        center = lineLineIntersectFlimsy(
...
/**\brief Compute the Poincaré disk coordinates of points along geodesics between pairs of hyperbolic plane coordinates.
  ...
  */
export function interpolateOnPoincareDisk(p0, p1, t, focus) {
  // zip from http://stackoverflow.com/questions/4856717/javascript-equivalent-of-pythons-zip-function:
  function zip(arrays) {
    return arrays[0].map(function(_,i){
      return arrays.map(function(array){return array[i]})
  ...
  const pout = pts.map(ppr => {
    ...
    if (pointsApproxCollinear(xd[0], xd[1], [0.0, 0.0], 1e-8)) {
      // Case where the arc has infinite radius (i.e., a line):
      pathCmd = vectorMAdd(pathCmd, vectorDiff(xd[1], xd[0], ppr[2]));
    } else {
      let center = [0., 0.]; // Center of circle forming geodesic between xd[0] & xd[1].
      if (bdy[0] && bdy[1]) {
        // Case where both points on disk boundary. Intersect tangents to get center.
      ...
      const ar = Math.sqrt(dx*dx + dy*dy);
      //pathCmd = pathCmd + ' A ' + ar + ',' + ar + ' 0 0,1 ' + xd[1][0] + ',' + xd[1][1];
      // FIXME: URHERE need to move along circular arc from xd[0] to xd[1] by fraction ppr[2].
    }
    return pathCmd;
  });
  return pout;
